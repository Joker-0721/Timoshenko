...
\pgfplotstableread{2d_ssss_vibration_q4_5x5_mindlin_exact_comparison.csv}{\dataFive}
\pgfplotstableread{2d_ssss_vibration_q4_17x17_mindlin_exact_comparison.csv}{\dataSeventeen}
\pgfplotstableread{2d_ssss_vibration_q4_5x5_17x17_selected_modes.csv}{\dataSelected}
...
比較 5×5 �?17×17 網格的有限元數值解�?SSSS Mindlin/Reissner 厚板解析解�?
使用 \(\Omega\) 可以消除尺寸、密度與彎曲剛度的量綱影響，比直接比�?Hz 更適合做�?benchmark 指標�?

\section{無因次頻率折線圖}

�?\ref{fig:omega-5x5} �?\ref{fig:omega-17x17} 分別比較 5×5 �?17×17 網格的有限元無因次頻�?\(\Omega_{\mathrm{FEM}}\) 與解析解 \(\Omega_{\mathrm{exact}}\)�?
...
�?\ref{fig:error-5x5} �?\ref{fig:error-17x17} 分別展示兩種網格�?\(|\epsilon_{\Omega}|\)（對數縱軸），以便觀察誤差的量級變化�?
5×5 網格在低階已有約 70\% 的誤差，高階誤差急劇增大；�?17×17 網格在整個計算範圍內誤差均低�?20\%�?
...
�?\ref{tab:selected-5x5} �?\ref{tab:selected-17x17} 分別摘錄�?5×5 �?17×17 網格中模式階�?1�?�?0�?0�?0�?0�?0�?0�?0�?5 的無因次頻率與誤差�?
其中 1 表示殘差通過�? 表示未通過�?
...
\pgfplotstabletypeset[
    ...
    columns/mode_rank/.style={column name=Mode, int},
    columns/m/.style={column name=$m$, int},
    columns/n/.style={column name=$n$, int},
    ...
    columns/residual_ok_5x5/.style={column name=殘差, int},
...
\pgfplotstabletypeset[
    ...
    columns/residual_ok_17x17/.style={column name=殘差, int},
    every head row/.style={before row=\toprule, after row=\midrule},
    every last row/.style={after row=\bottomrule},
    string type
]{\dataSelected}
...
\begin{itemize}
    ...
    \item \textbf{17×17 網格}：在整個計算的 500+ 階模態中，\(\Omega_{\mathrm{FEM}}\) 與解析解吻合良好，相對誤差始終低�?20\%，多數模態誤差小�?10\%。殘差檢查（欄位 \texttt{residual\_ok}）顯�?17×17 網格除前幾個模態外均滿足容差，�?5×5 網格在低階未通過（殘差容差設置較寬鬆時仍可能標記�?0，實際需根據程序設定判斷）�?
    \item \textbf{建議}：對於定量基準測試，推薦使用至少 17×17 �?Q4 網格�?×5 網格僅適合趨勢預分析，不能用於準確頻率計算�?
\end{itemize}
