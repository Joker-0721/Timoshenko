...
open("./date/new_vib/fem_tri3_SSSS_shear.csv", "w") do io
write(io, "lambda,lambda_real,k\n")

integrationOrder = 2
integrationOrder_s = 1
gmsh.initialize()
...
@timeit to "calculate ∫κκdΩ, ∫wwdΩ, ∫φφdΩ, ∫wφdΩ" begin
    ...
    @timeit to "get elements" elements_s = getElements(nodes, entities["Ω"],integrationOrder_s)
...
vtk_grid("./vtk/new_vib/fem_tri3_SSSS_shear.vtu", points, cells;
