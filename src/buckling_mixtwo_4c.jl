...
for n_div in 9:25
    @timeit to "Mixtwo Loop (ndiv=$n_div)" begin
        ...
        kᵅᵠᵠ = zeros(2 * nᵠ, 2 * nᵠ)   # 邊界罰項 φ
        kᵅʷʷ = zeros(nʷ, nʷ)           # 邊界罰項 w
        kʷʷ  = zeros(nʷ, nʷ)           # 幾何剛度 w
        ...
        all_φ_b = elements_φ_b[1] ∪ elements_φ_b[2] ∪ elements_φ_b[3] ∪ elements_φ_b[4]
        (∫αwwdΓ => all_w_b)(kᵅʷʷ, dummy_fʷ)
        (∫αφφdΓ => all_φ_b)(kᵅᵠᵠ, dummy_fᵠ)
