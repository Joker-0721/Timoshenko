# ==============================================================================
#  高級計算固體力學研究平台：Mindlin 屈曲分析單一網格混合法（厚度掃描專題）
#  流派：Meshfree-FE Hybrid Mixed Method (無網格-有限元高階多場混合變分原理)
#  特點：固定 17x17 結構化網格 + 厚度 h 自動化循環 (0.1 ➔ 0.00001)
...
# 🚀 規格固定：全面鎖定使用 17 階基礎網格檔案
const n_div = 17
mesh_file = "/home/a/Joker/msh/st_q_17.msh"

if !isfile(mesh_file)
    error("【錯誤】找不到指定的 17 階基礎網格檔案: ", mesh_file)
end

println("="^80)
println(" 啟動【單一網格混合法】厚度效應掃描大循環 (h = 0.1 ➔ 0.00001) ")
println("  固定使用網格: ", mesh_file)
println("="^80)
...
for h_val in h_series
    @timeit to "Total loop iteration (h=$h_val)" begin
        ...
        case_prefix = "buckling_mix_single_mesh_fixed17_h_$(h_val)"
        ...
        modes_csv_path = joinpath(DATA_DIR, "buckling_mix_st_q_17_h_$(h_val)_modes.csv")
        open(modes_csv_path, "w") do io
            ...
            for (rank, m_id) in enumerate(mode_ids[1:min(20, length(mode_ids))])
                ...
                ph1_v = d_m[1:2:2*nᵠ]; ph2_v = d_m[2:2:2*nᵠ]; w_v = d_m[2*nᵠ+1:end]
        ...
        vtk_grid(vtu_path, points, cells; ascii=true, append=false, compress=false) do vtk
            ...
            for (r, mode_id) in enumerate(mode_ids[1:n_modes_output])
                ...
                vᵠ_m = d_mode[1:2*nᵠ]; vʷ_m = d_mode[2*nᵠ+1:end]
                for i in 1:nˢ
                    p_id = nodes[i].𝐼
                    w_nodal[p_id]    = vʷ_m[i]
                    phi1_nodal[p_id] = vᵠ_m[2*i-1]; phi2_nodal[p_id] = vᵠ_m[2*i]
                end
        ...
        gmsh.finalize()
        println("  [SUCCESS] h = $(h_val) 屈曲計算成功. K_num = $(round(k_num, digits=4))")
    end
end
...
open(output_summary_filepath, "w") do io
    println(io, join(["ndiv", "h", "log10_h", "lambda_cr", "k_num", "relative_error_k", "log10_Error_L2_w", "log10_Error_L2_phi", "log10_Error_L2_Q"], ","))
    for row in convergence_table
        println(io, join([row.ndiv, row.h, row.log10_h, row.lambda_cr, row.k_num, row.rel_error_k, row.log10_Error_L2_w, row.log10_Error_L2_phi, row.log10_Error_L2_Q], ","))
    end
end

println(to)
println("="^80)
println("【厚度掃描重構成功】單一網格混合法總大表已寫入：", output_summary_filepath)
println("="^80)