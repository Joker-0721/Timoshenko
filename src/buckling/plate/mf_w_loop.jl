...
for bc in bcs
    ...
    ios[bc.name] = open("../date/mf/vibration/plate/vib_mf_w_φ_$(bc.name)_0.001.csv", "w")
...
for ndiv in ndivs
    ...
    for bc in bcs
        @timeit to "BC $(bc.name)" begin
            ...
            vtk_grid("../vtk/mf/vibration/$(bc.name)/vib_mf_w_φ_$(bc.name)_0.001.vtu", points, cells;
