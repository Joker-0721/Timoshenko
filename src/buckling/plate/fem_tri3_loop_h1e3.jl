...
for bc in bcs
    mkpath("../final_data/fem/vibration")
    ios[bc.name] = open("../final_data/fem/vibration/vib_fem_$(bc.name)_0.001.csv", "w")
...
for ndiv in ndivs
    ...
    for bc in bcs
        @timeit to "BC $(bc.name)" begin
            ...
            vtk_grid("../vtk/fem/vibration/$(bc.name)/vib_fem_$(bc.name)_0.001.vtu", points, cells;
