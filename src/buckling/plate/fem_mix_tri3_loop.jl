...
for bc in bcs
    mkpath("../date/fem/vibration/plate")
    ios[bc.name] = open("../date/fem/vibration/plate/vib_fem_mix_$(bc.name)_0.001.csv", "w")
...
for bc in bcs
    @timeit to "BC $(bc.name)" begin
        ...
        vtk_grid("../vtk/fem/vibration/$(bc.name)/vib_fem_mix_$(bc.name)_0.001.vtu", points, cells;
