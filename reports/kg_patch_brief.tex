...
對應程式是：
\begin{itemize}
  \item 宣告幾何剛度矩陣 \texttt{G\_ww}
  \item 加入幾何剛度 operator
  \item 組成
  \begin{equation*}
  K_g=
  \begin{bmatrix}
...
這次 patch test 的重點是：\textbf{在原本的材料剛度矩陣 $K$ 之外，加入了幾何剛度矩陣 $K_g$，並將求解式改成 $(K-K_g)d=f$。}  
從誤差比較來看，加入 $K_g$ 後，$w$ 與 $\phi$ 的誤差都有下降，其中 $w$ 的改善最明顯，表示加入 $K_g$ 後結果較接近理論解。
